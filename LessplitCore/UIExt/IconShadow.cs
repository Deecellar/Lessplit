...
namespace LessplitCore.UIExt
{
    public class IconShadow
    {
        ...
        public static Bitmap Generate(Image image, Color shadowColor)
        {
            ...
            using (var tempImage = new Bitmap(new Size(30, 30), PixelFormat.Format32bppRgba))
            {
                var resultImage = new Bitmap(new Size(30, 30), PixelFormat.Format32bppRgba);
                ...
                for (var x = 0; x < tempImage.Width; ++x)
                {
                    for (var y = 0; y < tempImage.Height; ++y)
                    {
                        ...
                        tempImage.SetPixel(x, y, Color.FromArgb((int)(alpha * result + 0.5f), (int)red, (int)green, (int)blue));
                    }
                }
                ...
                for (var x = 0; x < resultImage.Width; ++x)
                {
                    for (var y = 0; y < resultImage.Height; ++y)
                    {
                        ...
                        resultImage.SetPixel(x, y, Color.FromArgb((int)(shadowStrength * alpha * result + 0.5f), (int)red, (int)green, (int)blue));
                    }
                }
