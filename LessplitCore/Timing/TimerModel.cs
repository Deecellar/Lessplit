...
namespace LessplitCore.Timing
{
    ...
    {
        public SplitterState CurrentState
        {
            get
            {
        ...
        public void Start()
        {
            if (CurrentState.CurrentPhase == TimerState.NotRunning)
            {
                CurrentState.CurrentPhase = TimerState.Running;
        ...
        public void Split()
        {
            if (CurrentState.CurrentPhase == TimerState.Running && CurrentState.CurrentTime.RealTime > TimeSpan.Zero)
            {
                ...
                {
                    CurrentState.CurrentPhase = TimerState.Ended;
        ...
        {
            if ((CurrentState.CurrentPhase == TimerState.Running
                || CurrentState.CurrentPhase == TimerState.Paused)
        ...
        public void UndoSplit()
        {
            if (CurrentState.CurrentPhase != TimerState.NotRunning
                && CurrentState.CurrentSplitIndex > 0)
            {
                if (CurrentState.CurrentPhase == TimerState.Ended)
                    CurrentState.CurrentPhase = TimerState.Running;
                CurrentState.CurrentSplitIndex--;
                CurrentState.CurrentSplit.SplitTime = default(Time);
                CurrentState.Run.HasChanged = true;
        ...
        private void ResetSplits()
        {
            ...
            CurrentState.CurrentPhase = TimerState.NotRunning;
        ...
        public void Pause()
        {
            ...
            else if (CurrentState.CurrentPhase == TimerState.Paused)
            {
                CurrentState.AdjustedStartTime = TimeStamp.Now - CurrentState.TimePausedAt;
                CurrentState.CurrentPhase = TimerState.Running;
                OnResume?.Invoke(this, null);
            }
            else if (CurrentState.CurrentPhase == TimerState.NotRunning)
                Start(); //fuck abahbob                
        }

        public void UndoAllPauses()
        {
            if (CurrentState.CurrentPhase == TimerState.Paused)
                Pause();
